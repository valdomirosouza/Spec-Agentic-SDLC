...
**Status:** Accepted _(implemented 2026-09-13, #22: `.claude/hooks/sdd-gate.py`, `tests/hooks/test_sdd_gate.py`)_
**Date:** 2026-09-13
...
makes the agent _keep working_ without a human in the loop changes the autonomy model.

## Decision

1. **Adopt the `before_<command>` shape as a `UserPromptSubmit` hook, deterministic and
   read-only.** The script `.claude/hooks/sdd-gate.py` (#22) inspects `user_prompt`; when it starts with `/sdd-plan`, `/sdd-tasks`,
...
- Prompt-prefix detection is brittle (a prompt that merely _mentions_ `/sdd-implement` must
...
- **Review/expiry:** revisit when Cursor / Codex hook events are adopted.
