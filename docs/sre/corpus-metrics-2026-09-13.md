...
| Metric | Value | Method |
| --- | --- | --- |
| Commits on the default branch | 86 | git log over the full history; a day is active when it carries >= 1 commit |
| Active days | 2 (span 2 days) | — |
| Commits per active day | 43.0 | — |
| First / last commit | 2026-09-12T23:14:09 → 2026-09-13T20:03:20 | — |
...
| Metric | Value | Method |
| --- | --- | --- |
| Completed CI runs | 50 | the last 100 workflow runs; failure rate is failed/completed; recovery is the wall-clock gap from a failed run to the next successful one |
| Failed runs | 4 | — |
| Change failure rate | 8.0% | failed ÷ completed |
...
| --- | --- |
| Markdown files | 532 |
| Markdown lines | 60390 |
| Executable lines (scripts + hooks) | 3967 |
| Test lines | 1971 |
| Verification lines (scripts + hooks + tests) | 5938 |
| Prose to verification ratio | 10.2 : 1 |
...
| --- | ---: | ---: |
| Markdown files | 532 | 5 |
| Executable lines (scripts + hooks) | 3967 | 40 |
| Test lines | 1971 | 20 |
| Verification lines (scripts + hooks + tests) | 5938 | 59 |
...
| Metric | Value | Method |
| --- | --- | --- |
| Commits on the default branch | 86 | wall-clock from the first to the last commit on the default branch; this is elapsed time, not effort, and includes every pause |
| Elapsed wall-clock | 20.8 h | first commit to last |
| Commits per elapsed hour | 4.1 | — |
| Cumulative diff | 464 files changed, 16926 insertions(+), 1522 deletions(-) | `git diff --shortstat` from the first commit |
