...
<!-- Template. Copy to docs/ai-governance/model-card-<model-id>.md and fill every field.
     A filled instance is docs/ai-governance/model-card.md. Do not restate provider facts
     (training data, architecture, benchmarks) from memory — link the provider's published
     source instead (Constitution IX). -->
