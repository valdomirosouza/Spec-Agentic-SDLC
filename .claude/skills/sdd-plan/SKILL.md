...
1. **Setup**: `scripts/bash/check-prerequisites.sh --json --require-spec --require-approved`
   ...
   `scripts/bash/setup-plan.sh --json` → `IMPL_PLAN` (never overwrites an existing plan).
...
6. **Phase 0 → research.md**: one entry per unknown/dependency/integration with Decision,
   Rationale, Alternatives considered. Ground each claim (codebase → docs → Context7 → web →
   `uncertain — verify`).
7. **Phase 1 → data-model.md, contracts/, quickstart.md**: entities with fields, validation
   rules and state transitions; OpenAPI/AsyncAPI/Avro deltas (topics registered in
   `services.yaml`); runnable validation scenarios (prerequisites, commands, expected outcome —
